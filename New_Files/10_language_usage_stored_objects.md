...
is
    ...
    is
        k_ID        constant    employees.employee_id%type  := in_employee_id;
        r_employee              employees%rowtype;
    begin
        select  *
          into  r_employee
...
create package body employee_api
is
    -- ...
end employee_api;
/
...
as
    ...
    is
        ...
        function commission
            ...
            deterministic
        is
            k_SALARY    constant    employees.salary%type           := in_salary;
            k_COMM_PCT  constant    employees.commission_pct%type   := in_comm_pct;
        begin
...
is
    ...
    procedure del
        ( in_department_id  in  departments.department_id%type )
    is
        k_ID    constant    departments.department_id%type := in_department_id;
    begin
        if does_exist(in_department_id => k_ID) then
...
package employee_api
as
    g_salary_increase   types_up.sal_increase_type;  -- variabile globale pubblica
end employee_api;
/
```
...
as
    procedure set_salary_increase
        ( in_increase   in  types_up.sal_increase_type );

    function salary_increase
        return types_up.sal_increase_type;
end employee_api;
/
...
package body employee_api
as
    g_salary_increase   types_up.sal_increase_type;  -- privata nel body

    procedure set_salary_increase
        ( in_increase   in  types_up.sal_increase_type )
    is
        k_MIN   constant    types_up.sal_increase_type := constants_up.k_MIN_INCREASE;
        k_MAX   constant    types_up.sal_increase_type := constants_up.k_MAX_INCREASE;
    begin
        if in_increase between k_MIN and k_MAX then
            g_salary_increase := in_increase;
        end if;
...
begin
    g_salary_increase := constants_up.k_MIN_INCREASE;
...
```sql
-- Errato: se l_message non viene costruito correttamente, out_greeting rimane null
create or replace
package body my_package
is
    procedure greet
        (   in_name         in  employees.first_name%type
          , out_greeting    out varchar2
        )
    is
        k_NAME      constant    employees.first_name%type   := in_name;
        k_HELLO     constant    types_up.text               := 'Ciao, ';
        l_message               types_up.text;
    begin
        if k_NAME is not null then
            l_message    := k_HELLO || k_NAME || '!';
            out_greeting := l_message;
        end if;
        -- se k_NAME è null, out_greeting non viene mai assegnato
    end greet;
end my_package;
/
```
...
```sql
-- Corretto: out_greeting viene assegnato in ogni ramo
create or replace
package body my_package
is
    procedure greet
    ...
    is
        k_NAME      constant    employees.first_name%type   := in_name;
        k_HELLO     constant    types_up.text               := 'Ciao, ';
        k_UNKNOWN   constant    types_up.text               := 'Ospite';
    begin
        if k_NAME is not null then
            out_greeting := k_HELLO || k_NAME   || '!';
        else
            out_greeting := k_HELLO || k_UNKNOWN || '!';
        end if;
    end greet;
end my_package;
/
...
```sql
-- Errato: out_date impedisce l'uso della funzione in SQL
create or replace
package body my_package
is
    function my_function
...
begin
    ...
    my_package.do_something(in_id => l_employee_id);
end;
/
```
