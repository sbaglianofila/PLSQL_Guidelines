...
begin
    lib_logging.log(i_text => k_START, i_scope => k_SCOPE);
    -- elaborazione
    lib_logging.log(i_text => k_END,   i_scope => k_SCOPE);
end;
/
```
...
declare
    ...
    k_ROWNUM        constant    simple_integer              := 0;
    k_EMPTY_MSG     constant    lib_types.short_text_sbt    := 'empty_value';
    k_NDF_MSG       constant    lib_types.short_text_sbt    := 'no_data_found';
begin
    select  dummy
      into  l_dummy
      from  dual
...
exception
    when others then
        lib_logging.log_error(i_text => 'Eccezione non gestita');
        raise;
end;
/
```
...
-- la procedura lib_err.raise gestisce internamente raise_application_error
begin
    lib_err.raise(i_error => lib_err.k_INVALID_EMPLOYEE_ID);
end;
/
```
...
as
    function name_by_id
        ( i_id      in  departments.department_id%type )
        return departments.department_name%type
    is
        k_ID                constant    departments.department_id%type      := i_id;
...
as
    procedure discount_and_recalculate
        (   i_customer_id  in  customers.customer_id%type
          , i_discount     in  customers.discount_pct%type
        )
    is
        k_CUSTOMER_ID   constant    customers.customer_id%type      := i_customer_id;
        k_DISCOUNT      constant    customers.discount_pct%type     := i_discount;
        k_ERR_PREFIX    constant    lib_types.text_sbt               := 'Errore: ';
    begin
        pkg_customer.apply_discount(
              i_customer_id  => k_CUSTOMER_ID
            , i_discount     => k_DISCOUNT
        );
        pkg_customer.recalculate(i_customer_id => k_CUSTOMER_ID);
    exception
        when zero_divide then
            null;
        when others then
            lib_logging.log_error(i_text => k_ERR_PREFIX || sqlerrm);
            raise;
...
as
    ...
    is
        ...
        k_DISCOUNT      constant    customers.discount_pct%type     := i_discount;
        k_ERR_PREFIX    constant    lib_types.text_sbt               := 'Errore: ';
        k_BT_PREFIX     constant    lib_types.text_sbt               := ' - Backtrace: ';
    begin
        pkg_customer.apply_discount(
              i_customer_id  => k_CUSTOMER_ID
            , i_discount     => k_DISCOUNT
