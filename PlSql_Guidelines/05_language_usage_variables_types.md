...
Lo stesso principio vale per i sottotipi di uso comune: se in più punti del codice si usa un campo testuale da mille caratteri, è meglio definire un subtype `big_string_sbt` nel package dei tipi e usarlo ovunque, piuttosto che scrivere `varchar2(1000 char)` ogni volta.
...
is
    procedure my_proc
    is
        subtype big_string_sbt is varchar2(1000 char);
        l_note  big_string_sbt;
    begin
...
is
    procedure my_proc
    is
        l_note  lib_types.big_string_sbt;
    begin
        l_note := some_function();
        do_something(l_note);
    end my_proc;
...
Definire sottotipi semanticamente significativi è particolarmente utile per tipi trasversali al progetto: un tipo per gli identificatori numerici (`id_sbt`), uno per i nomi degli oggetti Oracle (`ora_name_sbt`), uno per le stringhe a lunghezza massima (`max_vc2_sbt`). Il subtype documenta l'intenzione d'uso e aggiunge un livello di significato che il tipo grezzo non trasmette.
...
is
    ...
    is
        ...
        k_HELLO         constant    lib_types.big_string_sbt    := 'Hello, ';
        l_last_name                 employees.last_name%type;
        l_message                   lib_types.big_string_sbt;
    begin
        select emp.last_name
          into l_last_name
          from employees  emp
        ...
        -- l_message costruita ma mai usata: pkg_message.send_message non viene chiamata
    exception
        when no_data_found
        then
            null;
...
is
    ...
    begin
        ...
        l_message := k_HELLO || l_last_name;
        pkg_message.send_message(l_message);
    exception
        when no_data_found
        then
            null;
...
declare
    l_note  lib_types.big_string_sbt := null;
begin
...
declare
    k_PARALLEL_DEGREE   constant    lib_types.name_sbt   := 'parallel_degree';
...
declare
    ...
    l_department_name               departments.department_name%type
                                    := pkg_department.name_by_id(i_id => k_DEPT_ID);
begin
...
begin
    <<init>>
    begin
        l_department_name := pkg_department.name_by_id(i_id => k_DEPT_ID);
    exception
        when value_error
        then
...
package lib_types
is
    subtype salary_sbt is number(8, 2);
end lib_types;
/
```
...
package lib_types
is
    subtype description_sbt is char(200);
end lib_types;
/
```
...
package lib_types
is
    subtype description_sbt is varchar2(200 char);
end lib_types;
/
```
...
package lib_types
is
    subtype description_sbt is varchar(200);
end lib_types;
/
```
...
is
    k_NULL_STRING   constant    lib_types.big_string_sbt    := '';
...
package lib_types
is
    subtype description_sbt is varchar2(200);
end lib_types;
/
```
...
begin
    ...
    then
        l_bigger := lib_constants.k_NUMERIC_TRUE;
    else
        l_bigger := lib_constants.k_NUMERIC_FALSE;
    end if;
