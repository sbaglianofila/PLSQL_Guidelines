...
as
    function department_salary
        ( i_dept_id     in  departments.department_id%type )
        return number
    is
        ...
        k_DEPT_ID       constant    departments.department_id%type  := i_dept_id;
    begin
...
as
    ...
    begin
        ...
        process_dept(i_dept_id => l_dept_id);  -- potrebbe azzerare sql%rowcount
...
as
    ...
    begin
        ...
        process_dept(i_dept_id => l_dept_id);
...
declare
    l_color     lib_types.color_code_sbt;
begin
    if l_color = lib_constants.k_RED then
        my_package.do_red();
    elsif l_color = lib_constants.k_BLUE then
        my_package.do_blue();
    elsif l_color = lib_constants.k_BLACK then
        my_package.do_black();
    end if;
end;
/
...
begin
    case l_color
        when lib_constants.k_RED   then my_package.do_red();
        when lib_constants.k_BLUE  then my_package.do_blue();
        when lib_constants.k_BLACK then my_package.do_black();
        else null;
    end case;
end;
/
...
select decode( ctry.country_code
             , lib_constants.k_CTRY_IT, lib_constants.k_LANG_ITALIAN
             , lib_constants.k_CTRY_FR, lib_constants.k_LANG_FRENCH
             , lib_constants.k_CTRY_DE, lib_constants.k_LANG_GERMAN
             ,                         lib_constants.k_LANG_NOT_SUPPORTED
             )
  from countries  ctry;
```
...
select case ctry.country_code
           when lib_constants.k_CTRY_IT then lib_constants.k_LANG_ITALIAN
           when lib_constants.k_CTRY_FR then lib_constants.k_LANG_FRENCH
           when lib_constants.k_CTRY_DE then lib_constants.k_LANG_GERMAN
           else                             lib_constants.k_LANG_NOT_SUPPORTED
       end
  from countries  ctry;
```
...
select case
           when ctry.country_code = lib_constants.k_CTRY_IT then lib_constants.k_LANG_ITALIAN
           when ctry.country_code = lib_constants.k_CTRY_FR then lib_constants.k_LANG_FRENCH
           when ctry.country_code is null                  then lib_constants.k_LANG_UNKNOWN
           else                                                 lib_constants.k_LANG_NOT_SUPPORTED
       end
  from countries  ctry;
```
...
select nvl( dummy, my_package.expensive_null(i_value => dummy) )
  from dual;
```

```sql
-- Corretto: expensive_null viene chiamata solo se dummy è null
select coalesce( dummy, my_package.expensive_null(i_value => dummy) )
  from dual;
```
...
select nvl2( dummy
           , my_package.expensive_nn  (i_value => dummy)
           , my_package.expensive_null(i_value => dummy)
           )
  from dual;
```
...
select case
           when dummy is null then my_package.expensive_null(i_value => dummy)
           else                    my_package.expensive_nn  (i_value => dummy)
       end
  from dual;
```
...
begin
    case l_color
        when lib_constants.k_RED  then my_package.do_red();
        when lib_constants.k_BLUE then my_package.do_blue();
        when lib_constants.k_RED  then my_package.do_black();  -- mai raggiunto
        else null;
    end case;
end;
/
...
if not l_color != lib_constants.k_RED then
    my_package.do_red();
end if;
```
...
declare
    k_STRING    constant    lib_types.text_sbt   := '42';
    l_is_valid              boolean;
begin
    l_is_valid := my_package.is_valid_number(i_string => k_STRING);

    if l_is_valid = true then
        my_package.convert_number(i_string => k_STRING);
    end if;
end;
/
```
...
as
    ...
    is
        k_PASSWORD      constant    lib_types.text_sbt           := i_password;
