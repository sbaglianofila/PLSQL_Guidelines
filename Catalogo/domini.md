...
## Colonne amministrative

Le sette colonne amministrative presenti su ogni tabella (`created_by`, `created_at`, `created_program`, `modified_by`, `modified_at`, `modified_program`, `row_version`) hanno tipi fissi, riconducibili ai domini di questo catalogo. Non vanno scelte caso per caso: la loro definizione completa — struttura, visibilità, trigger che le popola — è in `Architettura_DB/colonne_amministrative.md`, e il template di tabella le include già.

| Colonna | Dominio / Tipo | Note |
|---|---|---|
| `created_at`, `modified_at` | `datetime_val` (`timestamp(6)`) | Senza fuso; `datetime_tz` in progetti multi-regione |
| `created_by`, `modified_by`, `created_program`, `modified_program` | `varchar2(64 char)` (come `short_name`) | Allineato ai limiti di `CLIENT_IDENTIFIER` (64) e `MODULE` (48) |
| `row_version` | `number` | Contatore intero per il locking ottimistico |
