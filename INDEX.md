...
- [Architettura_DB/stati_workflow.md](Architettura_DB/stati_workflow.md) — *(bozza)* Pillar `wfl_` per gli stati come macchine a stati: `wfl_statuses` (stati con `is_initial`/`is_final`), `wfl_transitions` (transizioni ammesse, con `allowed_role`), audit dei cambi di stato con tabelle dedicate per entità (`wfl_<entità>_status_log`), e il package `lib_workflow` di validazione. Decise in questa passata la collocazione dell'audit (dedicato per entità), il prefisso `wfl_` e le guardie via `allowed_role`; motore di regole rimandato a bassa priorità; resta aperta la FK dalle colonne di stato verso `wfl_statuses`.
- [Architettura_DB/utenti_profili.md](Architettura_DB/utenti_profili.md) — *(bozza in consolidamento)* RBAC applicativo con tabelle `adm_*`: utenti (`adm_users`), ruoli aziendali (`adm_roles`), profili come insiemi di ruoli (`adm_profiles` + `adm_profile_roles`), maschere dedicate (`adm_masks`) con le funzionalità che le referenziano (`adm_functionalities`), funzionalità grantate ai profili con modi lettura/scrittura/esecuzione a flag (`adm_profile_functionalities`), assegnazione utente‑profilo (`adm_user_profiles`), e il package `lib_authz` di risoluzione dei permessi. Chiarita la distinzione dai ruoli di database di `schemi.md` e il legame con l'identità di sessione (`CLIENT_IDENTIFIER`). Decisioni prese: assegnazione e grant per profilo, `adm_masks` dedicata, autenticazione esterna, modi a flag; possibile estensione futura la gerarchia di funzionalità. Restano da produrre gli oggetti.
