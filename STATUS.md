...
| Stati / workflow | Pillar dedicato `wf_` (gli stati **non** sono lookup): `wf_statuses` (con `is_initial`/`is_final`), `wf_transitions` (transizioni ammesse come dato), audit dei cambi di stato per entità, package `lib_workflow` di validazione. **Documento in bozza**, decisioni ancora aperte |
...
| Architettura DB | `stati_workflow.md` (pillar `wf_` degli stati) | In corso (bozza da raffinare) |
...
- **Pillar stati/workflow (`wf_`)**: bozza in `stati_workflow.md`. Nodi aperti: dove vive l'audit dei cambi di stato (tabella generica con `entity_table`/`entity_id` vs dedicata per entità con FK tipizzata — la bozza propende per il dedicato); quanto ricche le transizioni (solo ruolo, o guardie/azioni/hook); prefisso dell'audit (`wf_` per coesione del pillar vs `log_` per natura di evento); FK composita dalle colonne di stato delle tabelle di business verso `wf_statuses`; e se `changed_by`/`changed_at` dell'audit vadano esplicite o derivate dalle colonne amministrative.
- **Companion API delle nuove strutture**: `lib_lookup` (lettura delle lookup, gemello di `lib_config`, con cache) e `lib_workflow` (`can_transition`/`change_status`) sono da aggiungere al catalogo dei pacchetti base e realizzare dopo la definizione delle rispettive tabelle.
